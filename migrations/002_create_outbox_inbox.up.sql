...
-- ⚠️ 实测踩坑（同 mdm-product/erp-inventory 的 002 迁移）：建分区要求
-- 执行者是父表的 owner，迁移用管理凭据跑，建出来的表默认属于那个账号；
-- 分区维护后台任务运行时用 erp_finance_rw（SET LOCAL ROLE 切换）建
-- 未来的分区，两者不是同一身份，必须显式把 owner 转过去。
ALTER TABLE event_outbox OWNER TO erp_finance_rw;
...
-- 本组件消费 sales.order.created.v1/erp.inventory.adjusted.v1/
-- mdm.customer.*，是本阶段消费面最重的一个组件（设计计划 §4）。
CREATE UNIQUE INDEX event_inbox_idem ON event_inbox (idempotency_key, created_at);

-- 同上：分区维护后台任务要能给 event_inbox 建未来的分区。
ALTER TABLE event_inbox OWNER TO erp_finance_rw;

-- 写命令的幂等表（命令侧，与事件消费侧的 event_inbox 分开）。
-- ClosePeriod/ReopenPeriod/LockPeriod/PostManualEntry/ReverseEntry 的
-- idempotency_key 落在这里去重；result_id 存该命令产生的业务标识
-- （期间操作 → period 状态，PostManualEntry/ReverseEntry → 凭证 id）。
